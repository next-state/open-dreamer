...
def load_i3d_pretrained(weights_path=None):
    ...
    2. dreamer/i3d_pretrained_400.npz (next to this file)
    3. fvd/videogpt/i3d_pretrained_400.pt (auto-convert)
    ...
    if weights_path is not None:
        ...
        npz_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                                'i3d_pretrained_400.npz')
    if not os.path.exists(npz_path):
        pt_path = os.path.join(os.path.dirname(os.path.abspath(__file__)),
                               '..', 'fvd', 'videogpt', 'i3d_pretrained_400.pt')
        if not os.path.exists(pt_path):
            raise FileNotFoundError(
                f"I3D weights not found at {npz_path}. "
                "Place i3d_pretrained_400.npz in the dreamer/ directory, or "
                "i3d_pretrained_400.pt in fvd/videogpt/ for auto-conversion.")
