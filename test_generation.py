...
def test_latent_rollout(checkpoint_dir: str, num_rollouts: int = 2, horizon: int = 4):
    ...
    mesh, _, mesh_rules = build_parallel("data")  # Uses CUDA_VISIBLE_DEVICES
    ...
    with jax.set_mesh(mesh):
        ...
        if actions_ctx.binary is not None and actions_ctx.categorical is not None:
            print(f"Context actions shape: binary {actions_ctx.binary.shape}, categorical {actions_ctx.categorical.shape}")
        if actions_future.binary is not None and actions_future.categorical is not None:
